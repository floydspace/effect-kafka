 ...
 */
export const make = (config: GlobalConfig): KafkaInstance.KafkaInstance =>
  KafkaInstance.make({
    producer: (options) =>
      Effect.gen(function* () {
 ...
 * @category layers
 */
export const layer = (config: GlobalConfig) => Layer.succeed(KafkaInstance.KafkaInstance, make(config));
